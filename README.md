</> Markdown

## Author
Arshalisha Abellera
Google Data Analytics Certificate - Capstone Project

GitHub: https://github.com/abelleraa98

# Youth Aging Out of Foster Care and the Risk of Homelessness: A Data Analysis

## Project Overview
Youth who age out of the foster care system face significant challenges as they transition into adulthood. Without permanent family support or stable housing, many former foster youth are at increased risk of experiencing homelessness. Understanding the relationship between youth aging out of foster care and youth homelessness can help inform policies and programs designed to support successful transitions to independent living. 

This project analyzes national data on youth aging out of foster care and youth homelessness in the United States. It also examines outcomes for youth transitioning out of foster care, including educational attainment, employment rates, housing stability, and access to healthcare coverage. By exploring these trends and outcomes, the project aims to better understand factors that may influence housing stability for youth leaving foster care. 

---

## Research Question
How do national trends in youth aging out of foster care correspond with trends in youth homelessness in the United States?

### Secondary Questions
1. How do employment rates among youth aging out of foster care relate to housing outcomes?
2. How does educational attainment among former foster youth compare to indicators of housing stability?
3. What level of healthcare coverage access exists for youth aging out of foster care, and how might this relate to access to support services?

---

## Variables Analyzed

- Youth homelessness rate
- Housing stability available after aging out
- Employment rates
- Educational attainment
- Access to mental health services

---

## Data Sources

1. U.S. Department of Housing and Urban Development (HUD)
  Office of Policy Development and Research
  Point-in-Time Estimates of Homelessness
2. U.S. Department of Health and Human Services (HHS)
  Administration for Children and Families (ACF)
  Child Welfare Outcomes Data - Exits to Emancipation
3. National Youth in Transition Database (NYTD)
  U.S. Department of Health and Human Services
  Cohort Outcome Reports

---

## Tools Used

- **Excel** - Data cleaning, organization, and creation of the master dataset
- **Github** - Project documentation and version control
- **SQL** - Data querying and analysis
- **Python** - Data analysis and visualization

---

## Analysis Plan
This project examines the relationship between youth aging out of foster care and youth homelessness in the United States. The analysis focuses on national trends and outcomes for youth transitioning out of the foster care system.

### 1. National Trends in Foster Care Exits and Youth Homelessness
This analysis examines national trends in youth aging out of foster care and youth homelessness between 2020 and 2024. The goal is to explore whether changes in the number of youth exiting foster care correspond with changes in youth homelessness. 

Variables analyzed:
- Youth Aging Out
- Youth Homeless 18-24

Data sources:
- U.S. Department of Health and Human Services (HHS), Administration for Children and Families (ACF)
- U.S. Department of Housing and Urban Development (HUD)

---

### 2. Housing Stability Outcomes Among Youth Aging Out of Foster Care
This analysis evaluates housing stability among youth who have transitioned out of foster care. Housing stability is measured using the percentage of youth reporting homelessness within 2022-2024.

Variables analyzed:
- Homeless Risk

Data source:
- National Youth in Transition Database (NYTD) Cohort Reports

---

### 3. Employment and Educational Outcomes
This analysis explores employment and educational attainment among youth aging out of foster care to better understand economic and educational factors that may influence housing outcomes. 

Variables analyzed:
- Employment Rate
- Educational Attainment

Data source:
- National Youth in Transition Database (NYTD)

---

### 4. Access to Health Coverage
This analysis examines access to healthcare coverage among youth aging out of foster care. Medicaid coverage is used as a proxy indicator for access to healthcare and mental health services.

Variables analyzed:
- Health Coverage Access

Data source:
- National Youth in Transition Database (NYTD) Cohort Reports

---

## Expected Visualizations 
The analysis will include visualizations that highlight trends and outcomes related to youth aging out of foster care and youth homelessness. 

1. **Youth Aging Out vs Youth Homelessness (2020-2024)**
  A line chart comparing national trends in youth exiting foster care through emancipation and    youth homelessness (ages 18-24)
2. **Housing Stability Outcomes**
  A bar chart showing the percentage of youth reporting homelessness within 2022-2024 for NYTD    Cohort 3 and Cohort 4.
3. **Employment and Educational Outcomes**
  A bar chart comparing employment rates and educational attainment among youth aging out of foster care.
4. **Access to Health Coverage**
  A bar chart showing the percentage of youth with Medicaid coverage at age 21.

---

## Repository Structure
```
data/
cleaned-data/
sql/
visualizations/
dashboard/
report/
README.md
```

---

## Purpose of the Project
This project was created as part of the Google Data Analytics Certificate Capstone Project. The objective is to demonstrate data analysis skills, including data cleaning, dataset integration, exploratory analysis, and data visualization.

---

## Future Work
- Expand analysis to multiple years of data
- Explore additional datasets related to social services
- Compare state-level housing policies and foster care outcomes
